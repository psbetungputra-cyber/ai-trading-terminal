...
/* ASFX_SIGNAL_HISTORY_PANEL_V1 */
(function(){
  if (window.__ASFX_SIGNAL_HISTORY_PANEL_V1__) return;
  window.__ASFX_SIGNAL_HISTORY_PANEL_V1__ = true;

  const styleId = "asfx-signal-history-panel-v1-style";
  ...
  function injectStyle(){
    ...
    style.textContent = `
      ...
      .asfx-signal-history-v1-refresh {
      ...
      .asfx-signal-history-v1-list {
        display: grid;
        gap: 8px;
      }

      .asfx-signal-history-v1-item {
        border: 1px solid rgba(148,163,184,.16);
      ...
      .asfx-signal-history-v1-status {
        font-size: 11px;
        font-weight: 950;
        color: #fff;
      }

      .asfx-signal-history-v1-meta,
      .asfx-signal-history-v1-sub {
        font-size: 11px;
        color: rgba(226,232,240,.72);
        line-height: 1.45;
      }

      .asfx-signal-history-v1-empty {
        font-size: 12px;
        color: rgba(226,232,240,.72);
        padding: 8px 0;
      }
  ...
  function timeValue(row){
    ...
    if (typeof raw === "object" && raw && typeof raw.seconds === "number") {
  ...
  function currentPair(){
    const title =
      document.querySelector(".asfx-detail-title h2")?.textContent ||
      document.querySelector('[data-asfx-bridge-rendered="signal"]')?.innerText ||
      "";
    const match = String(title).match(/\b[A-Z0-9]{5,20}\b/);
  ...
  function ensurePanel(){
    ...
    const signal = document.querySelector('[data-asfx-bridge-rendered="signal"]');
    if (!signal) return null;

    let panel = document.querySelector("[data-asfx-signal-history-v1]");
    if (!panel) {
      panel = document.createElement("div");
      panel.className = "asfx-signal-history-v1";
      panel.setAttribute("data-asfx-signal-history-v1", "true");
      signal.insertAdjacentElement("afterend", panel);
    }
  ...
  function renderState(message){
    ...
    panel.innerHTML = `
      <div class="asfx-signal-history-v1-head">
        <div class="asfx-signal-history-v1-title">Signal History</div>
        <button type="button" class="asfx-signal-history-v1-refresh" data-asfx-refresh-history>Refresh</button>
      </div>
      <div class="asfx-signal-history-v1-empty">${message}</div>
    `;
  }
  ...
  function renderRows(rows){
    ...
    if (!rows.length) {
      renderState("Belum ada snapshot tersimpan untuk pair ini.");
      return;
    }

    const items = rows.slice(0, 5).map((row) => {
      const status = clean(row.lifecycleStatus || row.status || row.signalStatus, "NO TRADE");
      const pair = clean(row.pair || row.symbol, "-");
      const tf = clean(row.timeframe || row.tf, "-");
      const confidence = clean(row.confidence, "-");
      const risk = clean(row.risk, "-");
      const zone = short(row.activeZone || row.entryZone || row.demandZone || row.supplyZone || "-");
      const when = displayTime(row);

      return `
        <div class="asfx-signal-history-v1-item">
          <div class="asfx-signal-history-v1-top">
            <div class="asfx-signal-history-v1-status">${status}</div>
            <div class="asfx-signal-history-v1-meta">${when}</div>
          </div>
          <div class="asfx-signal-history-v1-sub">${pair} · ${tf} · Confidence ${confidence} · Risk ${risk}</div>
          <div class="asfx-signal-history-v1-sub">Zone: ${zone}</div>
        </div>
      `;
    }).join("");
    ...
    panel.innerHTML = `
      ...
      <div class="asfx-signal-history-v1-list">${items}</div>
    `;
  }
  ...
  async function load(){
    ...
    const fb = window.AiSignalFirebase;
    if (!fb || typeof fb.getCollectionDocs !== "function") {
      renderState("Firebase read helper belum siap.");
      return false;
    }

    renderState("Loading signal history…");

    try {
      const rows = await fb.getCollectionDocs("signalSnapshots");
      const pair = currentPair();

      const filtered = (Array.isArray(rows) ? rows : [])
        .filter((row) => !pair || String(row.pair || row.symbol || "").toUpperCase() === pair)
        .sort((a, b) => timeValue(b) - timeValue(a));

      renderRows(filtered);
      return true;
    } catch (err) {
      console.warn("ASFX signal history load failed:", err);
      renderState("History belum bisa dibaca. Cek login atau Firestore rules.");
      return false;
    }
  }

  window.ASFX_SIGNAL_HISTORY_PANEL_V1 = {
    load,
    render: load
  };

  document.addEventListener("click", function(event){
    const btn = event.target.closest("[data-asfx-refresh-history]");
    if (!btn) return;
    event.preventDefault();
    load();
  });
  ...
  document.addEventListener("click", function(event){
    if (event.target.closest("[data-action='detail'], [data-detail-tab]")) {
      schedule();
    }
  }, true);

  const observer = new MutationObserver(() => {
    if (document.querySelector('[data-asfx-bridge-rendered="signal"]')) schedule();
  });

  if (document.body) observer.observe(document.body, { childList: true, subtree: true });

  schedule();
  console.info("ASFX Signal History Panel V1 ready.");
})();


/* ASFX_HISTORY_AUTO_REFRESH_AFTER_SAVE_V1 */
(function(){
  if (window.__ASFX_HISTORY_AUTO_REFRESH_AFTER_SAVE_V1__) return;
  window.__ASFX_HISTORY_AUTO_REFRESH_AFTER_SAVE_V1__ = true;

  const tryWrap = () => {
    const api = window.ASFX_FIREBASE_SIGNAL_SNAPSHOT_V1;
    if (!api || typeof api.save !== "function" || api.__historyAutoRefreshV1) return false;

    const originalSave = api.save.bind(api);

    api.save = async function(...args) {
      const result = await originalSave(...args);

      try {
        if (window.ASFX_SIGNAL_HISTORY_PANEL_V1?.load) {
          setTimeout(() => window.ASFX_SIGNAL_HISTORY_PANEL_V1.load(), 500);
          setTimeout(() => window.ASFX_SIGNAL_HISTORY_PANEL_V1.load(), 1800);
        }
      } catch (err) {
        console.warn("ASFX history auto refresh failed:", err);
      }

      return result;
    };

    api.__historyAutoRefreshV1 = true;
    console.info("ASFX History Auto Refresh After Save V1 ready.");
    return true;
  };
  ...
  if (!tryWrap()) {
    ...
    const timer = setInterval(() => {
      tries += 1;
      if (tryWrap() || tries > 40) clearInterval(timer);
    }, 500);
  }
})();
